...
struct ShadowMap
{
    ...
    void begin(int shadowMapResolution, CoreEngine* coreEngine, FullScreenQuad* fullScreenQuad, PerFrameUBO* perFrame, GLuint perFrameUBO)
    {
        ...
        // Create light camera
        lightCamera = new ThirdPersonCamera();
        lightCamera->setAspectRatio(shadowMapResolution, shadowMapResolution);
        lightCamera->setFarNearPlanes(1.0f, 100.0f);
    ...
    void display()
    {
        ...
        //// Prepare to render into blurred map FBO
        //glBindFramebuffer(GL_DRAW_FRAMEBUFFER, blurredMapFBO);
        //Utils::OpenGL::setRenderState(false, false, true);
        //glUseProgram(gaussianBlurXProgram);
        //fullScreenQuad->display();
